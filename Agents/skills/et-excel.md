...
这个 skill 负责通过 `ET.ExcelMcp` 的 CLI 入口读写 Excel、维护 Luban 配置表、做批量表格处理；如果要把表真正导出成 Luban 产物，改用 `et-luban`。
...
6. 改完表如果需要生成配置产物，再叠加 `et-luban`，不要把导出步骤混在本 skill 里。
