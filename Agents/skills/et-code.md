...
10. **严禁 AI 为了注册表、缓存、映射表等随手新增静态字段 / 静态字典 / 静态集合**；必须先确认生命周期与归属，优先使用 ET 的 `Singleton<T>`、组件、配置对象或已有框架注册机制。
...
- Attribute 扫描结果、节点注册表、配置缓存、运行时缓存等必须有明确 owner：优先放到 `Singleton<T>`、框架已有 Dispatcher、配置 Category、Scene/Fiber 组件或专门的管理组件中
- 如果确实需要静态只读常量，必须是不可变数据，并且不能承载运行时状态或热重载状态
- 写代码前如果出现“启动时扫描并保存到静态字典”“首次使用时 lazy init 静态缓存”这类设计，必须先改成 ET 生命周期对象；不能由 AI 自行创建静态字段
- 热重载、测试隔离、多 Fiber、多 Scene 场景下会复用进程级静态状态，默认应视为风险点
