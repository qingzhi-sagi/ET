...
foreach ($packageName in $packageNames) {
    $lines.Add(('    <Compile Include="$(ETRootDir)Packages/{0}/CodeMode/Config/Server/**/*.cs" Link="Packages\{0}\CodeMode\Config\Server\%(RecursiveDir)%(Filename)%(Extension)" />' -f $packageName))
}
