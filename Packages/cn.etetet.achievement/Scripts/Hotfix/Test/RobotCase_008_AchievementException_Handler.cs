...
namespace ET.Test
{
    ...
    public class RobotCase_008_AchievementException_Handler : ARobotCaseHandler
    {
        ...
        private static void CreateBasicExceptionTestData(AchievementComponent achievementComponent)
        {
            ...
            Log.Debug(
                "Created 4 test achievements for exception test: 1001(Kill,InProgress), 2001(Level,Completed), 3001(Quest,InProgress), 4001(Exploration,Claimed)");
        }
        ...
        private static async ETTask TestGetNonExistentAchievementDetail(Scene robotScene)
        {
            ...
            {
                Log.Debug(
                    "Warning: Expected error for non-existent achievement, but got success. This might be acceptable if default data is returned.");
            }
            else
            {
        ...
        private static async ETTask TestClaimIncompleteAchievement(Scene robotScene)
        {
            ...
            Log.Debug(
                $"Found incomplete achievement: {incompleteAchievement.AchievementId} with progress {incompleteAchievement.Progress}/{incompleteAchievement.MaxProgress}");
        ...
        private static async ETTask TestGetInvalidCategoryAchievements(Scene robotScene)
        {
            ...
            Log.Debug(
                $"Get achievements with negative category ID result: {response1.Error} - {response1.Message}, Count: {response1.Achievements?.Count ?? 0}");
