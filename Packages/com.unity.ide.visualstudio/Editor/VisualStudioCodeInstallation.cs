	...
	internal class VisualStudioCodeInstallation : VisualStudioInstallation {
...
#if UNITY_EDITOR_OSX
			return Directory.Exists(path) && Regex.IsMatch(path, ".*Visual Studio Code.*.app$", RegexOptions.IgnoreCase);
		...
		public static bool TryDiscoverInstallation(string editorPath, out IVisualStudioInstallation installation) {
			...
			installation = new VisualStudioCodeInstallation() {
				...
				Name = "Visual Studio Code" + (isPrerelease ? " - Insiders" : string.Empty) + (version != null ? $" [{version.ToString(3)}]" : string.Empty),
				Path = editorPath,
...
";
		...
		private Process FindRunningVSCodeWithSolution(string solutionPath) {
			...
			foreach (var process in processes) {
				try {
					...
					Debug.LogError($"[VSCode] Error checking process: {ex}");
					continue;
				}
			}
			return null;
		...
		public override bool Open(string path, int line, int column, string solution) {
			...
			var existingProcess = FindRunningVSCodeWithSolution(directory);
			if (existingProcess != null) {
				try {
					...
					Debug.LogError($"[VSCode] Error using existing instance: {ex}");
				}
			}
