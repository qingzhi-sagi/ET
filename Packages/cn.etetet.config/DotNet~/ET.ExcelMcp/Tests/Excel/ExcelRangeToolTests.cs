...
public class ExcelRangeToolTests : ExcelTestBase
{
    ...
    public async Task MoveRange_ShouldMoveRangeToDestination()
    {
        ...
        string sourceA1Value;
        using (var package = new ExcelPackage(new FileInfo(workbookPath)))
        {
            sourceA1Value = package.Workbook.Worksheets[0].Cells["A1"].Value?.ToString() ?? "";
        }
    ...
    public async Task WriteRange_WithObjectFormat_ShouldWriteData()
    {
        ...
        using var package = new ExcelPackage(new FileInfo(outputPath));
        var worksheet = package.Workbook.Worksheets[0];
        Assert.Equal(10.0, Convert.ToDouble(worksheet.Cells["A1"].Value));
    ...
    public async Task WriteRange_WithNumericValues_ShouldStoreAsNumbers()
    {
        ...
        using var package = new ExcelPackage(new FileInfo(outputPath));
        var worksheet = package.Workbook.Worksheets[0];
        Assert.Equal(100.0, Convert.ToDouble(worksheet.Cells["A1"].Value));
    ...
    public async Task ClearRange_WithClearFormat_ShouldClearFormat()
    {
        ...
        // Assert
        using var package2 = new ExcelPackage(new FileInfo(outputPath));
        var resultStyle = package2.Workbook.Worksheets[0].Cells["A1"].Style;
        Assert.False(resultStyle.Font.Bold);
    }
    ...
    public async Task CopyRange_WithValuesOnly_ShouldCopyValuesOnly()
    {
        ...
        arguments["destRange"] = "B1";
        arguments["copyOption"] = "values";
        ...
        // Assert
        using var package2 = new ExcelPackage(new FileInfo(outputPath));
        var resultWorksheet = package2.Workbook.Worksheets[0];
        Assert.Equal("Test", resultWorksheet.Cells["B1"].Value);
        var destStyle = resultWorksheet.Cells["B1"].Style;
        Assert.False(destStyle.Font.Bold);
    }
    ...
    public async Task CopyFormat_ShouldCopyFormatOnly()
    {
        ...
        var destStyle = resultWorksheet.Cells["B1"].Style;
        Assert.True(destStyle.Font.Bold, "Format should be copied (bold should be true)");
    ...
    public async Task CopyFormat_WithCopyValue_ShouldCopyFormatAndValues()
    {
        ...
        using var package2 = new ExcelPackage(new FileInfo(outputPath));
        var resultWorksheet = package2.Workbook.Worksheets[0];
        Assert.Equal("Original", resultWorksheet.Cells["B1"].Value);
        var destStyle = resultWorksheet.Cells["B1"].Style;
    ...
    {
        // Arrange
        var workbookPath = CreateExcelWorkbook("test_write_no_range.xlsx");
        var outputPath = CreateTestFilePath("test_write_no_range_output.xlsx");
        var arguments = CreateArguments("write", workbookPath, outputPath);
        arguments["data"] = new JsonArray { new JsonArray { "A" } };
