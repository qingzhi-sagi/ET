...
3. 该分支必须使用Unity**6000.0.54**(初学者请在此版本用熟后再切换其他版本)
...
8.  安装Powershell，主要ET的各种工具都会使用Powershell来编写，方便跨平台。比如ET->Excel->ExportExcel 默认是用powershell来调用luban导出配置的
9.  点击Unity菜单 Assets->Open C# Project，这里由于修改了rider插件，会自动打开ET.sln 
10. Unity中打开ET.Init包中的GlobalConfig，设置SceneName为StateSync，然后点击设置主包
11. 刚clone下来，unity打开可能会有些报错，注意先确保安装了il2cpp。
12. 执行Unity菜单ET->Init->RefreshAssemblyReference,这里会刷新unity的asmdef reference, 如果unity中找不到这个菜单，可以用命令行dotnet ./Bin/ET.CodeMode.dll
13. 编译整个ET.sln, 注意要翻墙，否则可能nuget包下载不下来，导致编译出错(翻墙后如果还有报错解决不了可以尝试先用VS打开ET.sln编译一次后再回到Rider重新编译一次)
14. Unity中, 然后双击Packages/ET.StateSync/Scenes/Init场景，点击Play(▶)即可运行
15. 如果出现StringHashHelper.GetLongHashCode方法重复的错误，这是因为编译了SourceGenerater.dll，unity修改了SourceGenerater.dll.meta文件，请在git中还原这个文件即可
16. 独立启动服务器，右键UnityHub，以管理员身份运行UnityHub，然后启动Unity（没有管理员启动是不行的，因为服务端要开启http服务，普通权限开不了），
    停止Unity Play，点开Unity菜单->ET->Server Tools->Start Server(Single Process)，这样就单独启动了服务端。打开Unity菜单 -> ET -> BuildTool中CodeMode改成Client，点击Unity Play，登录。如果还是连接不上报10037错误，注意看ET/Logs目录，看有没有Error日志。 如果要用rider启动服务器，rider也必须用管理员权限启动
...
17. 注意独立运行服务器的目录不再是Bin目录，而是Bin的上一层目录，也就是Unity目录，比如 dotnet.exe Bin/ET.App.dll --Console=1。用rider启动默认是在Bin目录，需要自己修改运行目录，去掉Bin
