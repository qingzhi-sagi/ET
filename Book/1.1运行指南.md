...
3. 请自己安装dotween跟odin插件
4. 该分支必须使用Unity**2022.3.62**(初学者请在此版本用熟后再切换其他版本)
5. 整个过程请开启全局翻墙，否则各种unity包 nuget包下载不下来，报memerypack等错误
6. 注意一定要clone一个新的工程！clone一个新的工程！clone一个新的工程！重要的事情说三遍！！！
7. 启动UnityHub，打开(Open) -> 选中'ET'文件夹所在目录后打开工程
8. 打开工程后，点击Unity菜单 -> Edit -> Preferences -> External Tools，点击下拉框'External ScriptEditor'选择Rider，Generate .csproj files for要勾选前两个
9. 编译luban，用rider打开Packages/cn.etetet.yiuiluban/DontNet~/luban/src/Luban.sln, 编译完成之后才能导出Excel配置
10. 点击Unity菜单 Assets->Open C# Project，这里由于修改了rider插件，会自动打开ET.sln 
11. Unity中打开ET.Init包中的GlobalConfig，设置SceneName为StateSync，然后点击设置主包
12. 刚clone下来，unity打开可能会有些报错，注意先确保安装了il2cpp。
13. 编译ET.sln中的ET.CodeMode跟ET.SourceGenerater工程
14. 执行Unity菜单ET->Init->CodeMode,这里会刷新unity的asmdef reference, 如果unity中找不到这个菜单，可以用命令行dotnet ./Bin/ET.CodeMode.dll
15. 编译整个ET.sln, 注意要翻墙，否则可能nuget包下载不下来，导致编译出错(翻墙后如果还有报错解决不了可以尝试先用VS打开ET.sln编译一次后再回到Rider重新编译一次)
16. Unity中, 然后双击Packages/ET.StateSync/Scenes/Init场景，点击Play(▶)即可运行
17. 如果出现StringHashHelper.GetLongHashCode方法重复的错误，这是因为编译了SourceGenerater.dll，unity修改了SourceGenerater.dll.meta文件，请在git中还原这个文件即可
18. 独立启动服务器，右键UnityHub，以管理员身份运行UnityHub，然后启动Unity（没有管理员启动是不行的，因为服务端要开启http服务，普通权限开不了），
